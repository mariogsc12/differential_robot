...
void loop() {
  ...
  if(current_millis - last_millis >= interval)
  {
    ...
    if (millis()>=5000)input = 200;     
    if (millis()>=10000)input = 210;     
    if (millis()>=15000)input = 220;     
    if (millis()>=20000)input = 0;

    Serial.print(millis());Serial.print(",");
    Serial.print(input);Serial.print(",");
    Serial.println(right_wheel_meas_vel);
    //Serial.print("left_wheel: ");Serial.println(left_wheel_meas_vel);

    last_millis = current_millis;
    right_encoder_counter = 0;
    left_encoder_counter = 0;

    analogWrite(L298N_enA, abs(saturate(input, -255, 255)));
    //analogWrite(L298N_enB, abs(saturate(150, -255,255)));
  }
}
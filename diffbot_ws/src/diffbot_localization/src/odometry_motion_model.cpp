#include "diffbot_localization/odometry_motion_model.hpp"
...
double angle_diff(const double &a_, const double &b_)
{
  double a = atan2(sin(a_), cos(a_));
  double b = atan2(sin(b_), cos(b_));
                    ...
                    , is_first_odom_(true)
{
  ...
  odom_sub_ = create_subscription<nav_msgs::msg::Odometry>("diffbot_controller/odom", 10, std::bind(&OdometryMotionModel::odomCallback, this, _1));
